...
RUN apt-get update && DEBIAN_FRONTEND=noninterative apt-get install -y wget curl

# add our user and group first to make sure their IDs get assigned consistently
RUN groupadd -r kibana && useradd -r -m -g kibana kibana

RUN apt-get update && DEBIAN_FRONTEND=noninterative apt-get install -y \
        apt-transport-https \
        ca-certificates \
        wget \
        libfontconfig \
...
RUN apt-get update && DEBIAN_FRONTEND=noninterative apt-get install -y git
...
RUN curl --silent https://deb.nodesource.com/gpgkey/nodesource.gpg.key | apt-key add - \
 && apt-get update && DEBIAN_FRONTEND=noninterative apt-get install -y lsb-release \
 && export VERSION=node_6.x \
 && export DISTRO="$(lsb_release -s -c)" \
 && echo "deb https://deb.nodesource.com/$VERSION $DISTRO main" | tee /etc/apt/sources.list.d/nodesource.list \
 && echo "deb-src https://deb.nodesource.com/$VERSION $DISTRO main" | tee -a /etc/apt/sources.list.d/nodesource.list \
 && apt-get update \
 && DEBIAN_FRONTEND=noninterative apt-get install -y nodejs 
...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninterative apt-get install -y build-essential make libkrb5-dev gcc pkg-config libcairo2-dev libpng-dev libjpeg-dev libgif-dev g++ gyp node-gyp ruby ruby-dev rpm
